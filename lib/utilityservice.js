...
UtilityService.prototype.setVlocitySetting = async function(settingName, value) {
    try {    
        let result = await this.vlocity.jsForceConnection.query(`Select Id, ${this.vlocity.namespacePrefix}Value__c from ${this.vlocity.namespacePrefix}GeneralSettings__c where Name = '${settingName}'`);
...
UtilityService.prototype.getVlocitySetting = async function(settingName) {
    try {    
        let result = await this.vlocity.jsForceConnection.query(`Select ${this.vlocity.namespacePrefix}Value__c from ${this.vlocity.namespacePrefix}GeneralSettings__c where Name = '${settingName}'`);
...
UtilityService.prototype.getVisualForcePageUrl = function (dataPackType, dataPackObj, dataPackId) {
    ...
    if (dataPackType === 'OmniScript' && dataPackObj['%vlocity_namespace%__IsLwcEnabled__c']) {
